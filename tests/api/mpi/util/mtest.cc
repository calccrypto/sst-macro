 ...
 */
void
MTest_Init_thread(int *argc, char ***argv, int required, int *provided)
{
  int flag;
  const char *envval = 0;
  ...
  if (envval)
  {
    if (strcmp(envval, "yes") == 0 || strcmp(envval, "YES") == 0 || strcmp(
        envval, "true") == 0 || strcmp(envval, "TRUE") == 0)
    {
      returnWithVal = 1;
    }
    else if (strcmp(envval, "no") == 0 || strcmp(envval, "NO") == 0 || strcmp(
        envval, "false") == 0 || strcmp(envval, "FALSE") == 0)
    {
      returnWithVal = 0;
    }
    else
 ...
 */
static void *
MTestTypeContigInit(MTestDatatype *mtype)
{
  MPI_Aint size;
  int merr;
 ...
 */
static void *
MTestTypeContigInitRecv(MTestDatatype *mtype)
{
  MPI_Aint size;
  int merr;
...
static void *
MTestTypeContigFree(MTestDatatype *mtype)
{
  if (mtype->buf)
  {
    free(mtype->buf);
...
static int
MTestTypeContigCheckbuf(MTestDatatype *mtype)
{
  unsigned char *p;
...
static void *
MTestTypeVectorFree(MTestDatatype *mtype)
{
  if (mtype->buf)
  {
    free(mtype->buf);
 ...
 */
static void *
MTestTypeIndexedInitRecv(MTestDatatype *mtype)
{
  MPI_Aint totsize;
  int merr;
...
static void *
MTestTypeIndexedFree(MTestDatatype *mtype)
{
  if (mtype->buf)
  {
    free(mtype->buf);
...
static int
MTestTypeIndexedCheckbuf(MTestDatatype *mtype)
{
  unsigned char *p;
  unsigned char expected;
  int i, err = 0, merr;
 ...
 */
int
MTestGetDatatypes(MTestDatatype *sendtype, MTestDatatype *recvtype, int count)
{
  int merr;
  int i;
  ...
  case 3:
    ...
    if (merr)
      MTestPrintError(merr);
    merr = MPI_Type_set_name(sendtype->datatype, (char*) "dup of MPI_INT");
    if (merr)
      MTestPrintError(merr);
  ...
  case 4:
    ...
    merr = MPI_Type_vector(recvtype->count, 1, 3, MPI_INT, &sendtype->datatype);
    if (merr)
      MTestPrintError(merr);
    merr = MPI_Type_commit(&sendtype->datatype);
    if (merr)
      MTestPrintError(merr);
    merr = MPI_Type_set_name(sendtype->datatype, (char*) "int-vector");
    if (merr)
      MTestPrintError(merr);
    sendtype->count = 1;
    recvtype->datatype = MPI_INT;
  ...
  case 5:
    ...
    if (merr)
      MTestPrintError(merr);
    merr = MPI_Type_set_name(sendtype->datatype, (char*) "int-indexed(4-int)");
    if (merr)
      MTestPrintError(merr);
    sendtype->count = 1;
  ...
  case 7:
    ...
    merr = MPI_Type_indexed(recvtype->nelm, recvtype->index, recvtype->displs,
  ...
  else if (verbose && datatype_index > 0)
  {
    printf("Get new datatypes: send = %s, recv = %s\n",
        MTestGetDatatypeName(sendtype), MTestGetDatatypeName(recvtype));
    fflush(stdout);
  }
 ...
 */
int
MTestGetIntracommGeneral(MPI_Comm *comm, int min_size, int allowSmaller)
{
  int size, rank, merr;
  int done = 0;
  int isBasic = 0;
  ...
  while (!done)
  {
    ...
    case 3:
      ...
      if (merr)
        MTestPrintError(merr);
      merr = MPI_Comm_split(MPI_COMM_WORLD,
          ((rank < size / 2) ? 1 : MPI_UNDEFINED), size - rank, comm);
      if (merr)
        MTestPrintError(merr);
 ...
 */
int
MTestGetIntracomm(MPI_Comm *comm, int min_size)
{
...
{
  int merr;
  if (*comm != MPI_COMM_WORLD && *comm != MPI_COMM_SELF && *comm
      != MPI_COMM_NULL)
  {
    merr = MPI_Comm_free(comm);
    if (merr)
...
void
MTestPrintErrorMsg(const char msg[], int errcode)
{
  int errclass, slen;
  char string[MPI_MAX_ERROR_STRING];
 ...
 */
void
MTestPrintfMsg(int level, const char format[], ...)
{
  va_list list;
  int n;
...
MTestResourceSummary(FILE *fp)
{
  ...
  if (getrusage(RUSAGE_SELF, &ru) == 0)
  {
    ...
    if (pfThreshold == -2)
    {
      const char *p = getenv("MPITEST_RUSAGE_PF");
      pfThreshold = -1;
      if (p)
      {

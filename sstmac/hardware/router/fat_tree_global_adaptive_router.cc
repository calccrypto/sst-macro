...
fat_tree_global_adaptive_router::cheapest_path(
    ...
    std::list <geometry_routable::path> cheapest;
    for(std::list <geometry_routable::path> const & p : paths){
        int this_queue_length = 0;

        // add up queue lengths
        for(geometry_routable::path const & hop : p){
